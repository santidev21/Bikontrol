...
- Emails are sent through the SMTP settings in `.env` (`Smtp__Host`, `Smtp__Port`, `Smtp__Username`, `Smtp__Password`, `Smtp__FromEmail`, `Smtp__FromName`, `Smtp__EnableSsl`). If SMTP is not configured, the reset link is logged to the console so the flow can still be tested locally.
